...
!   use distribution, only: 
  ...
  interface
    subroutine print_ben ( n ) bind ( c )
      use iso_c_binding
      integer ( c_int ), VALUE :: n
    end subroutine print_ben
    subroutine ben_init ( n ) bind ( c )
      use iso_c_binding
      integer ( c_int ), VALUE :: n
...
call ben_timer_end("comp_flux_vel_ghost")
!-----------------------------------------------------------------------
!
...
!  call print_ben(2)
 ...
 write ( *, * ) 'BEN nblocks_clinic=', nblocks_clinic
 write ( *, * ) 'BEN km=', km



!BEN added barrier here to get more accurate timing
...
!BEN added barrier here to get more accurate timing
if (k == 1) call MPI_Barrier(POP_communicator,ierr)
call ben_timer_start()
...
call ben_timer_end("tracer_update")
...
call ben_timer_end("rest_of_k_loop")
...
call ben_timer_end("implicit_vertical_mix")
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
 ...
 write ( *, * ) 'BEN (before momentum equations) nblocks_clinic=', nblocks_clinic
 write ( *, * ) 'BEN (before momentum equations) km=', km
...
call ben_timer_end("state before clinic")
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
    ...
    call ben_timer_end("store_forces_in_UVEL_and_VVEL")
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
    ...
    call ben_timer_end("increment_sum_for_v-avged_forcing")
!-----------------------------------------------------------------------
!
...
call ben_timer_end("normalize_sums_for_v-avgs")
!-----------------------------------------------------------------------
!
...
call ben_timer_start()
      if (implicit_vertical_mix)                   &
         call impvmixu(UVEL(:,:,:,newtime,iblock), &
                       VVEL(:,:,:,newtime,iblock), & 
                       this_block)
call ben_timer_end("if implicit_vertical_mix")
!-----------------------------------------------------------------------
!
...
call ben_timer_end("calc unnormalized bc velocities")
!-----------------------------------------------------------------------
!
...
call ben_timer_end("find v-avgs")
!-----------------------------------------------------------------------
!
...
call ben_timer_start()
      do k = 1,km
         ...
         endwhere
      enddo
call ben_timer_end("normalize_baroclinic_velocities")
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
!EOC
call ben_timer_end("rest_of_baroclinic")
