      ...
      if (hs_local) then
         ...
         do ipe2=1, ceil2num
            ...
            if ( send_bl(ipe)%type /= MPI_DATATYPE_NULL ) then
               if (ipe-1 /= comm_pid) &
                  call MPI_IRECV ( signal, 1, mp_i4, ipe-1, comm_pid, comm, hs_rcvids(ipe), ierr )
            endif
         enddo
      endif
...
!
        if ( recv_bl(ipe)%type /= MPI_DATATYPE_NULL ) then
          ...
          if (hs_local) then
             if (ipe-1 /= comm_pid) &
               call MPI_SEND ( signal, 1, mp_i4, ipe-1, ipe-1, comm, ierr )
          endif
        endif
      enddo
...
!
        if ( send_bl(ipe)%type /= MPI_DATATYPE_NULL ) then
          ...
          if (hs_local) then
             if (ipe-1 /= comm_pid) &
                call MPI_WAIT ( hs_rcvids(ipe), Status, ierr )
             if (send_local) then
         ...
         do ipe2=1, ceil2num
            ...
            if (qsize .ne. 0) then
               ...
               if (r8_win%dest /= comm_pid) &
                  call MPI_IRECV ( signal, 1, mp_i4, r8_win%dest, send_tag, comm, hs_rcvids(ipe), ierr )
            endif
         enddo
      endif
            ...
            if (onetwo*unitsize >= offset_r-offset_0) then
              ...
              if (hs_local) then
                 if (r8_win%src /= comm_pid) &
                   call MPI_SEND ( signal, 1, mp_i4, r8_win%src, recv_tag, comm, ierror)
              endif
            else
         ...
         do ipe2=1, ceil2num
            ...
            if (qsize .ne. 0) then
               ...
               if (i4_win%dest /= comm_pid) &
                  call MPI_IRECV ( signal, 1, mp_i4, i4_win%dest, send_tag, comm, hs_rcvids(ipe), ierr )
            endif
         enddo
      endif
            ...
            if (onetwo*unitsize >= offset_r-offset_0) then
              ...
              if (hs_local) then
                 if (i4_win%src /= comm_pid) &
                   call MPI_SEND ( signal, 1, mp_i4, i4_win%src, recv_tag, comm, ierror)
              endif
            else
...
! Post initial handshake receive requests
         if (handshake) then
            do istep=1,maxreq
               p = swapids(istep)
               if (sndlths(p) > 0) then
                  call mpi_irecv  ( hs, 1, mp_i4, p, comm_pid, comm, &
...
! Post initial receive requests
         do istep=1,maxreq
            ...
            if (rcvlths(p) > 0) then
               ...
               if (handshake) then
                  call mpi_send( hs, 1, mp_i4, p, p, comm, &
                                 ierr )
               endif
            endif
         enddo
...
! Submit new i(r)send request
            ...
            if (sndlths(p) > 0) then
               if (handshake) then
                  call mpi_wait( hs_rcvids(istep), Status, ierr )
                  if (sendd) then
...
! Submit new i(r)send request
            ...
            if (sndlths(p) > 0) then
               if (handshake) then
                  call mpi_recv  ( hs, 1, mp_i4, p, comm_pid, comm, &
                                   OutStats, ierr )
                  if (sendd) then
                     call mpi_rsend( ga_r8_s(offset_s), sndlths(p), mp_r8, &
                                      p, comm_pid, comm, ierr )
