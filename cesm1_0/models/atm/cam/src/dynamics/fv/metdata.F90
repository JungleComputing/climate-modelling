...
! !USES
  ...
  use time_manager,       only: get_curr_date, get_step_size, get_calendar
  ...
  use parutilitiesmodule, only: parcollective, BCSTOP, parscatter
#endif
  use perf_mod
...
! !INPUT PARAMETERS:
    ...
    call open_met_datafile( grid, curr_filename, curr_fileid, curr_data_times, check_dims=.true. )

    if ( len_trim(next_filename) > 0 ) &
         call open_met_datafile( grid, next_filename, next_fileid, next_data_times )
...
!------------------------------------------------------------------------
  subroutine read_next_metdata(grid)
    ...
    do i=1,2
       call infld(Tname, fids(i), 'lon', 'lev', 'lat',  1, pcols, 1, pver, &
       ...
       call infld(Qname, fids(i), 'lon', 'lev', 'lat',  1, pcols, 1, pver, &
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
  subroutine open_met_datafile( grid, fname, fileid, times, check_dims )
...
!              31  28  31  30  31  30  31  31  31  31  30  31
    ...
    if ( get_calendar() == 'GREGORIAN' ) then
