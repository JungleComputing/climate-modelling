...
use ESMF_Mod
use shr_kind_mod, only : SHR_KIND_CS, SHR_KIND_CL, SHR_KIND_IN, SHR_KIND_R8
...
subroutine esmfshr_util_ArrayCopy(srcArray, dstArray, rc)
    ...
    do j = srcLB(2), srcUB(2)
        do i = srcLB(1), srcUB(1)
            if(src2dst_map(i) /= -1) then
               im = src2dst_map(i-srcLB(1)+1) + dstLB(1) - 1
               dstFptr(im, j) = srcFptr(i, j)
            endif
        enddo
    enddo

    deallocate(src2dst_map)
