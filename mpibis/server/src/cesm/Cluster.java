...
public class Cluster { 
	...
	synchronized void addConnection(int rank, int size, String name, Connection c) throws Exception { 
		...
		if (rank < 0 || rank >= connections.length) { 
			throw new Exception("Illegal rank " + rank + " for cluster " + name + " of size " + localCount);
		}

		if (localCount != size) { 
			throw new Exception("Size of cluster does not match (" + size + " != " + localCount + ")");			
		}
		
		if (!this.name.equals(name)) { 
			throw new Exception("Name of cluster does not match (" + name + " != " + this.name + ")");			
		}
		
		if (connections[rank] != null) { 
			throw new Exception("Rank " + rank + " on cluster " + name + " already in use!");				
		}

		connections[rank] = c;			
		connectionCount++;
		
		System.out.println("Added connection " + rank + " to " + name);		
	}			
